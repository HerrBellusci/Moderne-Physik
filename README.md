...
- **multibib** (`_extensions/pandoc-ext/multibib/`, ISC-Lizenz, [github.com/pandoc-ext/multibib](https://github.com/pandoc-ext/multibib)): Pandoc-Lua-Filter für das mehrteilige Literaturverzeichnis (Grundlagen / weiterführende Literatur / Literatur zur Konzeption). Die Quellen liegen in `references-grundlagen.bib`, `references-weiterfuehrend.bib` und `references-konzeption.bib`; `references-parked.bib` sammelt vorgemerkte, noch unzitierte Einträge und wird nicht gerendert. Reiner Build-Schritt, das gerenderte HTML lädt dadurch keinen zusätzlichen Code. Hinweis: `multibib.lua` ist lokal gepatcht (liest den Metadaten-Schlüssel `multibib:`, weil Quartos YAML-Validierung die Map-Form von `bibliography:` ablehnt); nach einem Update der Erweiterung muss dieser Patch neu angewendet werden, sonst bleiben die Literatur-Abschnitte leer.
