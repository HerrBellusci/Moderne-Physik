...
local function resolve_doc_citations (doc)
  ...
  local meta = doc.meta
  -- Quarto validiert `bibliography` als String; darum alternativ `multibib` erlauben
  local bibconf = meta.multibib or meta.bibliography
...
local function create_topic_bibliography (div)
  ...
  local bibfile = name and (doc_meta.multibib or doc_meta.bibliography or {})[name]
  if not bibfile then
    return nil
  end
