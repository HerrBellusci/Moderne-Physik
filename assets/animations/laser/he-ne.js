(function () {
  ...
  function math(p, S, x, y, tex, fallback, size = 24, color = C.ink, anchor = 'start') {
    if (!window.SRTMath?.label(p, x, y, tex, size, anchor, color))
      text(p, S, x, y, fallback, size, color, anchor);
  }
  ...
  function arrow(p, S, x1, y1, x2, y2, color, extra = {}) {
    ...
    S.el('path', { d: `M${x2-d*Math.cos(a-.5)} ${y2-d*Math.sin(a-.5)} L${x2} ${y2} L${x2-d*Math.cos(a+.5)} ${y2-d*Math.sin(a+.5)}`, fill:'none', stroke:color, 'stroke-width':3, ...extra, 'stroke-dasharray':'none', 'stroke-linecap':'round', 'stroke-linejoin':'round' }, p);
  }
  function setup({ parent:p, SRT:S }) {
    ...
    line(p,S,211,61,562,61,C.ink);
    // Schaltzeichen einer Spannungsquelle, Polung für den Gleichspannungsbetrieb.
    S.el('circle',{cx:388,cy:61,r:28,fill:'white',stroke:C.ink,'stroke-width':2.5},p);
    line(p,S,360,61,416,61,C.ink,{'stroke-width':2.5});
    text(p,S,338,52,'−',26,C.ink,'middle'); text(p,S,439,52,'+',26,C.ink,'middle');
    text(p,S,388,25,'Hochspannungsquelle',24,C.ink,'middle');
    text(p,S,62,116,'Kathode (−)',23);
    arrow(p,S,182,125,206,153,'#111111',{'stroke-width':2.5});
    text(p,S,597,116,'Anode (+)',23);
    arrow(p,S,595,125,570,153,'#111111',{'stroke-width':2.5});
    ...
    math(p,S,744,190,'632{,}8\\,\\mathrm{nm}','632,8 nm',23,C.red,'middle');
  }
  function levels({ parent:p, SRT:S, state }) {
    S.clear(p);
    S.el('rect',{width:862,height:560,fill:'white'},p);
    text(p,S,263,78,'Helium',28,C.ink,'middle');
    text(p,S,643,78,'Neon',28,C.ink,'middle');
    // Schematische Energieachse; der Maßstabshinweis steht in der Bildunterschrift.
    arrow(p,S,76,500,76,66,C.muted);
    math(p,S,51,88,'E','*E*',27,C.ink,'end');
    math(p,S,51,117,'\\mathrm{eV}','eV',22,C.muted,'end');
    math(p,S,61,507,'0','0',22,C.muted,'end');
    ...
    math(p,S,156,137,'20{,}61\\,\\mathrm{eV}','20,61 eV',23,C.amber);
    text(p,S,156,198,'metastabil',23,C.amber);
    math(p,S,510,113,'20{,}66\\,\\mathrm{eV}','20,66 eV',22,C.red);
    text(p,S,617,113,'· oberes Laserniveau',22,C.red);
    math(p,S,510,315,'18{,}70\\,\\mathrm{eV}','18,70 eV',22);
    text(p,S,617,315,'· unteres Laserniveau',22);
    ...
    math(g,S,425,241,'+\\,0{,}05\\,\\mathrm{eV}','+ 0,05 eV',21,C.amber,'middle');
    arrow(g,S,323,222,323,484,C.amber,{'stroke-dasharray':'8 6','stroke-linecap':'round'});
    ...
    text(g,S,602,201,'Laserübergang',22,C.red);
    math(g,S,602,228,'632{,}8\\,\\mathrm{nm}','632,8 nm',22,C.red);
    g=group(4);
    arrow(g,S,555,330,555,380,C.blue,{'stroke-width':4});
    text(g,S,577,355,'spontane Emission',22,C.blue);
    text(g,S,577,382,'in tiefere Zustände',22,C.blue);
    line(g,S,522,397,603,397,C.muted,{'stroke-dasharray':'5 5'});
    arrow(g,S,555,412,555,484,C.muted,{'stroke-dasharray':'8 6','stroke-linecap':'round'});
    text(g,S,577,435,'weitere Übergänge',21,C.muted);
    text(g,S,577,462,'und Stöße',21,C.muted);
  }
