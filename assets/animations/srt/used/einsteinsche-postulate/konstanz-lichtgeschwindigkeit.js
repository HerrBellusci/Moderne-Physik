(() => {
  ...
  document.querySelectorAll("[data-zp2-animation]").forEach((root) => {
    ...
    function geometry() {
      // Fester Ausschnitt für beide Systeme und alle Reglerwerte.
      // Links bleibt Platz für Erdquelle, Baum und Richtungspfeil,
      // rechts auch für den klassischen Vergleichspuls bei c + v.
      const left = 116;
      const right = width - 26;
      const distance = Math.max(1, (right - left) / (1 + 2 * maxSpeed));
      return {
        startX: left + maxSpeed * distance,
        lightSpeed: distance / TRAVEL_TIME,
        topY: HEIGHT * 0.32,
        bottomY: HEIGHT * 0.7
      };
    }
    ...
    function draw() {
      ...
      const { startX, lightSpeed, topY, bottomY } = geometry();
      ...
      let opacity = 1;
      if (state.playing && cyclePosition > LEAD_TIME + travelTime) {
      ...
      if (state.frame === "earth") {
        shipMuzzle = startX + state.speed * lightSpeed * emissionTime;
        groundMuzzle = startX;
      } else {
        shipMuzzle = startX;
        groundMuzzle = startX - state.speed * lightSpeed * emissionTime;
      }
      const pulseX = startX + lightSpeed * lightTime;
      ...
      const topLabel = state.frame === "earth" && state.speed > 0
        ? "bewegte Lichtquelle" : "ruhende Lichtquelle";
      const bottomLabel = state.frame === "earth" || state.speed === 0
        ? "Vergleich: ruhende Lichtquelle"
        : "Vergleichsquelle bewegt sich";
      context.fillStyle = "#5a6678";
      context.font = `600 17px ${fontFamily}`;
      context.textAlign = "right";
      context.fillText(topLabel, width - 24, topY - 64);
      context.fillText(bottomLabel, width - 24, bottomY - 64);
      ...
      if (fired) {
        ...
        drawClassicalExpectation(startX, lightSpeed, lightTime, topY, bottomY);
      }
    ...
    function drawClassicalExpectation(startX, lightSpeed, lightTime, topY, bottomY) {
      if (!state.showClassical || state.speed <= 0) return;

      const inEarthFrame = state.frame === "earth";
      const ghostX = startX + lightSpeed
      ...
      context.font = `italic 17px ${fontFamily}`;
    ...
    function updateSpeedValue() {
      const sign = state.speed === 0 ? "" : state.frame === "earth" ? "+" : "\u2212";
