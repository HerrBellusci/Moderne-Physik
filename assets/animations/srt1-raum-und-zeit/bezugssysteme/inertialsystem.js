(function () {
  ...
  function drawUniformSystem(parent, SRT, x, y, w, drift) {
    ...
    for (let i = 0; i < 22; i++) {
      const sx = innerX + 10 + positiveModulo(i * 47, innerW - 20);
      const sy = innerY + 14 + positiveModulo(i * 71, innerH - 28);
      SRT.el("circle", { cx: sx, cy: sy, r: i % 3 === 0 ? 2.5 : 1.8, fill: "#fff", opacity: 0.72 }, space);
    }

    // Gleichförmige Bewegung vor ruhendem Hintergrund. Der Umlauf erfolgt
    // vollständig außerhalb des sichtbaren Ausschnitts, einschließlich Rand.
    const offscreenMargin = 16;
    const travelWidth = innerW + 2 * offscreenMargin;
    const ox = innerX - offscreenMargin
      + positiveModulo(innerW / 2 + offscreenMargin + drift, travelWidth);
    const oy = innerY + innerH / 2;
    SRT.el("circle", { cx: ox, cy: oy, r: 13, fill: "#e9eef5", stroke: "#172033", "stroke-width": 3 }, space);
    drawReferencePoint(space, SRT, ox, oy);
  }
